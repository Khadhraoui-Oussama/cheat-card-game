...
const JoinGameSection = () => {
	...
	const handleJoinRoomWithCode = () => {
		...
		if (
			...
			player.name.length < 21 &&
			player.avatar.length > "/avatars/.svg".length
		) {
		...
		<div>
			...
			{playerInfoMissingAlert && (
				<Alert key="danger" variant="danger">
					Some input fields are missing , please Make sure to enter a valid room code,
					select an avatar and choose a name between 3 and 20 characters
				</Alert>
			)}
			{joinAlert && (
				<Alert key="danger" variant="danger">
					The room you are trying to join is full , please try another room or create your
					own game.
				</Alert>
			)}
		</div>
...
//FIND A WAY TO START THE GAME WITH THE CONFIG SELECTED IN THE GAMEOPTIONS
